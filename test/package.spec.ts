...
describe('test', async () => {
  ...
  type TestMode = 'ci' | 'integration' | 'unit';
  function executeSuiteWithProfile(mode: TestMode) {
    //skips a given test suite if env TEST_PROFILE is set and equals to 'mode' param
    //if TEST_PROFILE is not set - suite will not be skiped (this is to execute all tests by default or in local env)
    if (process.env.TEST_PROFILE && process.env.TEST_PROFILE != mode) {
      this.skip();
    }
  }

  describe('unit tests', async () => {
    executeSuiteWithProfile('ci'); //execute this suite with TEST_MODE=ci (or if TEST_MODE is not present)
  });
  describe('integration tests', async () => {
    executeSuiteWithProfile('integration'); //execute this suite with TEST_MODE=integration (or if TEST_MODE is not present)
    it('make manager', async () => {
      await userService.createUser(managerLogin, managerPass, [
        AppRole.MANAGER
      ]);
    ...
    it('delete users', async () => {
      ...
      const anotherUser = await userRepository.getUserByLogin(
        anotherUserForTest
      );
    ...
    describe('proxy', async () => {
      ...
      it('get cashed hotel info without prices', async () => {
        ...
        expect(res.body).to.be.a('object');
        expect(res.body.hotelId).to.be.eq(providerAccommodationId);
      }).timeout(10000);
      ...
      it('check booked with simard api', async () => {
        ...
        const amadeusDeal = deals.find(
          (v) => v.offerId === amadeusPricedOfferId
        );
    ...
    describe('group booking', async () => {
      ...
      it('request a group booking', async () => {
        ...
        expect(
          Number(res.body.depositOptions.offerCurrency.amount)
        ).to.be.above(100);
        expect(res.body.depositOptions.offerCurrency.currency).to.be.equal(
          'EUR'
        );
